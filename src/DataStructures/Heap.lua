--Taken from https://github.com/Roblox/Wiki-Lua-Libraries/blob/master/StandardLibraries/Heap.lua
--8/11/2023 klc adding UpdateIndex() method 
...
--Added by klc 8/11/2023
function Heap:UpdateIndex(index, newValue)
	if #self <= 1 then
		self[index] = newValue
		return
	end

	if self.Compare(self[index], newValue) then
		self[index] = newValue
		SiftUp(self, index)
	else
		self[index] = newValue
